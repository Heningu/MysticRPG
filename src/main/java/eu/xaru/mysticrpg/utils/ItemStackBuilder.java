...
public class ItemStackBuilder {
    ...
    public ItemStackBuilder withDisplayName(String displayName) {
        this.displayName = Utils.getInstance().$(displayName);
        return this;
    }

    public ItemStackBuilder withLore(List<String> lore) {
        this.lore = lore.stream().map(Utils.getInstance()::$).toList();
        return this;
    }
    ...
    public ItemStack build() {
        ...
        if (material == XMaterial.PLAYER_HEAD && owner != null) {
            DebugLogger.getInstance().log("Creating player head with owner: " + owner);
            itemStack = new ItemStack(Material.PLAYER_HEAD);
            SkullMeta meta = (SkullMeta) itemStack.getItemMeta();
            meta.setOwningPlayer(Bukkit.getOfflinePlayer(owner));
            meta.getPersistentDataContainer().set(new NamespacedKey(MysticCore.getInstance(), "uuid"), PersistentDataType.STRING, owner.toString());
        ...
        if (!lore.isEmpty()) {
            meta.setLore(lore.stream().map(Utils.getInstance()::$).toList());
        }
