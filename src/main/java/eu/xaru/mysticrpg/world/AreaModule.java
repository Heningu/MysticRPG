        ...
        new CommandAPICommand("area")
                .withSubcommand(new CommandAPICommand("create")
                        ...
                        .executesPlayer((player, args) -> {
                            ...
                            if (areaHelper.getAreaById(areaName) != null) {
                                player.sendMessage("§cError: An area with the name '" + areaName + "' already exists.");
                                return;
                            }
                            ...
                            player.sendMessage("§aUse §b/area set 1 §aand §b/area set 2 §ato set corners for area: §e" + areaName);
                        }))
                .withSubcommand(new CommandAPICommand("set")
                        ...
                        .executesPlayer((player, args) -> {
                            ...
                            if (!areaHelper.isInModifyState(player)) {
                                player.sendMessage("§cYou must create an area first using §b/area create <name>§c.");
                                return;
                            }
                            Location currentLocation = player.getLocation();
                            Location[] corners = areaHelper.getPlayerAreaCorners(player);
                            if (corners[0] != null && corners[1] != null) {
                                player.sendMessage("§cBoth corners are already set. Use §b/area clear §cto reset them first.");
                                return;
                            }
                            if (corner.equals("1")) {
                                areaHelper.setPlayerAreaCorner(player, 0, currentLocation);
                                player.sendMessage("§aFirst corner set at your current location.");
                            } else if (corner.equals("2")) {
                                areaHelper.setPlayerAreaCorner(player, 1, currentLocation);
                                player.sendMessage("§aSecond corner set at your current location.");
                                ...
                                if (corners[0] != null && corners[1] != null) {
                                    ...
                                    player.sendMessage("§aArea §e" + areaName + " §ahas been successfully saved.");
                                }
                            } else {
                                player.sendMessage("§cInvalid corner. Use '1' or '2'.");
                            }
                        }))
                .withSubcommand(new CommandAPICommand("modify")
                        ...
                        .executesPlayer((player, args) -> {
                            String areaName = (String) args.get("name");
                            if (areaHelper.getAreaById(areaName) == null) {
                                player.sendMessage("§cArea not found: §e" + areaName);
                                return;
                            }
                            areaHelper.enterModifyState(player, areaName);
                            player.sendMessage("§aYou are now modifying area: §e" + areaName + "§a. Use §b/area clear §ato reset corners, then §b/area set 1 §aand §b/area set 2 §ato define new corners. Use §b/area save §ato save changes.");
                        }))
                .withSubcommand(new CommandAPICommand("clear")
                        .executesPlayer((player, args) -> {
                            if (!areaHelper.isInModifyState(player)) {
                                player.sendMessage("§cYou must be in modify mode to clear an area's corners.");
                                return;
                            }
                            areaHelper.setPlayerAreaCorner(player, 0, null);
                            areaHelper.setPlayerAreaCorner(player, 1, null);
                            player.sendMessage("§aCleared the corners for the area. Use §b/area set 1 §aand §b/area set 2 §ato set new corners.");
                        }))
                .withSubcommand(new CommandAPICommand("save")
                        .executesPlayer((player, args) -> {
                            if (!areaHelper.isInModifyState(player)) {
                                player.sendMessage("§cYou must be in modify mode to save an area.");
                                return;
                            }
                            String areaName = areaHelper.getModifyStateAreaName(player);
                            Location[] corners = areaHelper.getPlayerAreaCorners(player);
                            if (corners[0] == null || corners[1] == null) {
                                player.sendMessage("§cYou must set both corners before saving the area.");
                                return;
                            }
                            areaHelper.modifyArea(areaName, corners[0], corners[1]);
                            areaHelper.exitModifyState(player);
                            player.sendMessage("§aArea §e" + areaName + " §ahas been successfully modified and saved.");
                        }))
                .withSubcommand(new CommandAPICommand("list")
                        .executesPlayer((player, args) -> {
                            List<AreaHelper.SpawnArea> areas = areaHelper.getAllAreas();
                            if (areas.isEmpty()) {
                                player.sendMessage("§cNo areas have been defined.");
                            } else {
                                player.sendMessage("§aDefined Areas:");
                                for (AreaHelper.SpawnArea area : areas) {
                                    player.sendMessage("§e- " + area.getAreaId());
                                }
                            }
                        }))
                .withSubcommand(new CommandAPICommand("rule")
                        ...
                        .executesPlayer((player, args) -> {
                            ...
                            if (action.equalsIgnoreCase("list")) {
                                if (!areaHelper.isInModifyState(player)) {
                                    player.sendMessage("§cYou must be in modify mode to list area rules.");
                                    return;
                                }
                                String areaName = areaHelper.getModifyStateAreaName(player);
                                AreaHelper.SpawnArea area = areaHelper.getAreaById(areaName);
                                player.sendMessage("§aRules for area §e" + areaName + "§a:");
                                for (Map.Entry<String, Boolean> entry : area.getFlags().entrySet()) {
                                    player.sendMessage("§e- " + entry.getKey() + ": §b" + entry.getValue());
                                }
                            } else if (action.equalsIgnoreCase("set")) {
                                if (!areaHelper.isInModifyState(player)) {
                                    player.sendMessage("§cYou must be in modify mode to edit area rules.");
                                    return;
                                }
                                ...
                                player.sendMessage("§aRule §e" + rule + " §ahas been set to §b" + state + " §afor area §e" + areaName);
                            }
                        }))
                .register();
    }
